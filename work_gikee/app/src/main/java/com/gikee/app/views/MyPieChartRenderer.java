...
public class MyPieChartRenderer extends PieChartRenderer {
    ...
    /**
     * 这个方法，左边是从下往上画，右边是从上往下画 * @param c
     */
    @Override
    public void drawValues(Canvas c) {
        Rect rect = new Rect();
        getPaintEntryLabels().getTextBounds(text, 0, text.length(), rect);
        int textHeight = rect.height() + 5;//文本的高度
        MPPointF center = mChart.getCenterCircleBox(); // get whole the radius
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            final PieDataSet.ValuePosition yValuePosition = dataSet.getYValuePosition(); // apply the text-styling defined by the DataSet applyValueTextStyle(dataSet);
            ...
            for (int j = 0; j < entryCount; j++) {
                ...
                final float sliceSpaceMiddleAngle = sliceSpace / (Utils.FDEG2RAD * labelRadius); // offset needed to center the drawn text in the slice
                ...
                if (drawXOutside || drawYOutside) {
                    ...
                    final float pt1y = labelRadius * (1 + valueLineLength1) * sliceYBase + center.y; //左右算法不一样,左边是从下往上排的，即你可以理解为饼图是顺时针方向，从零点排到12点的360度圆形，建议先看else里的，即右边的，方便理解
                    if (transformedAngle % 360.0 >= 90.0 && transformedAngle % 360.0 <= 270.0) {//左边部分 //
                        pt2x = pt1x - polyline2Width;
                        pt2x = center.x - 3 * radius / 2 - 5;
                        LogUtils.showLog("左边部分pt2x=" + pt2x + "^^^^^^" + radius + "%%%%%%%%%%" + MyUtils.getWidth());
                        ...
                        if (drawXOutside) getPaintEntryLabels().setTextAlign(Paint.Align.RIGHT);
                        labelPtx = pt2x - offset;
                        labelPty = pt2y;
                    } else {//右边部分 //
                        pt2x = pt1x + polyline2Width;
                        pt2x = center.x + 3 * radius / 2 + 5;
                        LogUtils.showLog("右边部分pt2x=" + pt2x + "^^^^^^" + 3 * radius / 2);
                        ...
                        if (drawXOutside) getPaintEntryLabels().setTextAlign(Paint.Align.LEFT);
                        labelPtx = pt2x + offset;
                        labelPty = pt2y;
                    }
                ...
                if (drawXInside || drawYInside) { // calculate the text position
                    float x = labelRadius * sliceXBase + center.x;
                    float y = labelRadius * sliceYBase + center.y;
                    mValuePaint.setTextAlign(Paint.Align.CENTER); // draw everything, depending on settings
